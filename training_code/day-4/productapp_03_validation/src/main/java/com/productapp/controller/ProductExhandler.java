...
@RestControllerAdvice// use aop : throwAdvice
public class ProductExhandler {

	private Logger logger=LoggerFactory.getLogger(ProductExhandler.class);
	
	//handler to handle PNFEx
	...
	public ErrorInfo handle404(ProductNotFoundException ex) {
		...
		logger.error("ex product not found "+ ex.getMessage());
		return info;
